...
// 双向广搜版本
int search(queue<int>& q, queue<int>& qb
, unordered_map<int, int>& dist, unordered_map<int, int>& distb) {
    int t = q.front(); q.pop();
    ...
    for(int i = 0; i < map[a].size(); i++) {
        ...
        for(int j = 0; j < map[b].size(); j++) {
            ...
            for(int k = 0; k < map[c].size(); k++) {
                ...
                if(distb.count(next)) return dist[t] + distb[next] + 1;
                dist[next] = dist[t] + 1;
                q.push(next);
            }
        }
...
int bfs() {
    ...
    while(qa.size() || qb.size()) {
        int ans;
        if(qa.size() && (ans = search(qa, qb, dista, distb)) != -1) return ans;
        if(qb.size() && (ans = search(qb, qa, distb, dista)) != -1) return ans;
    }
    return -1;
}
